...
subtitle: "[Deep Learning Specialization: Course 1 of 5](https://www.coursera.org/learn/neural-networks-deep-learning)"
...
```{r Scale With Labeled Dat, echo=FALSE, fig.width=10, fig.height=6}
x <- seq(from = 2, to = 50, by = 1)

dplyr::tibble(
  `Amount of Labeled Data (m)` = x,
  `Traditional Model`      = (x - 1) / sqrt(1 + x * x),
  `Small Neural Network`   = `Traditional Model` * 1.07,
  `Medium Neural Network`  = `Traditional Model` * 1.15,
  `Large Neural Network`   = `Traditional Model` * 1.25
) %>% tidyr::pivot_longer(
  -`Amount of Labeled Data (m)`, 
  names_to = "Model Type", 
  values_to = "Performance"
) %>% ggplot2::ggplot(
  ggplot2::aes(x = `Amount of Labeled Data (m)`, y = `Performance`, group = `Model Type`, color = `Model Type`)
) + ggplot2::geom_line() +
